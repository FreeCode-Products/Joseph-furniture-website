...
export default function ProductDetailPageClient({
  ...
  const product = providedProduct ?? getProductBySlug(slug);
  ...
  const galleryImages = product
    ? [product.imagePath, ...(product.gallery ?? [])].filter(Boolean)
    : [];
    ...
    <div className="min-h-screen bg-cream pt-24 sm:pt-28 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <motion.nav
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex flex-wrap items-center gap-2 text-sm text-charcoal/50 mb-8"
        >
          ...
          <span>/</span>
          <span className="text-charcoal max-w-[70vw] sm:max-w-none truncate">
            {product.name}
          </span>
        </motion.nav>
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-start">
          ...
          >
            <div className="relative h-[300px] sm:h-[400px] lg:h-[550px] rounded-2xl overflow-hidden bg-bone">
              <Image
                src={galleryImages[selectedImage] ?? product.imagePath}
                alt={product.name}
                fill
                priority
              ...
              <div className="mt-4 grid grid-cols-5 gap-2">
                {galleryImages.map((img, i) => (
                  <button
                    ...
                    className={`relative aspect-square rounded-lg overflow-hidden bg-bone border-2 transition-all ${
                      selectedImage === i
                        ? "border-walnut"
                        : "border-transparent hover:border-walnut/40"
                    }`}
          ...
          >
            <div>
              ...
              <h1 className="text-2xl sm:text-4xl md:text-5xl font-heading font-bold text-charcoal mb-2">
                {product.name}
              </h1>
            </div>

            <p className="text-base sm:text-lg text-charcoal/60 leading-relaxed">
              {product.description}
            </p>
              ...
              <div className="flex flex-wrap gap-3">
                {product.colors.map((color, i) => (
                  <button
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm border transition-all ${
                      selectedColor === i
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <Button
                size="lg"
                variant="outline"
                className="h-14 w-full sm:w-14 rounded-full border-sand/50 hover:bg-sand/20"
          ...
          <section className="mt-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {relatedProducts.map((rp, i) => (
                ...
                >
                  <Link
                    href={`/products/view?slug=${rp.slug}`}
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          <button
                            ...
                            onClick={(event) => {
                              ...
                              window.open(
                                buildProductWhatsAppUrl(rp, { origin }),
                                "_blank",
                                "noopener,noreferrer"
                              );
                            }}
